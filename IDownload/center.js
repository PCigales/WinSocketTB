...
async function set_progress(sdid) {
  ...
  if (download.className == "working") {
    const estim = progress.estimation ?? 0;
    download.getElementsByClassName("estimation")[1].innerText = dur_form.format({days: Math.floor(estim / 86400), hours: Math.floor((estim % 86400) / 3600), minutes: Math.floor((estim % 3600) / 60), seconds: estim % 60});
  } else {
...
function send_command() {
  ...
  switch (this.className) {
    ...
    case "suspend":
      ...
      break;
    case "restart":
      if (download.className != "aborted" || (Object.hasOwn(progress, "sections") && ! window.confirm("Restart the download ?"))) {break;}
      delete progress.sections;
      ...
      download.getElementsByClassName("percent")[0].innerText = progress.size ? "0" : "";
      download.getElementsByClassName("estimation")[1].innerText = "";
    case "resume":
      if (download.className != "aborted") {break;}
      download.className = "";
      browser.storage.local.get(["p0_" + sdid, "p1_" + sdid]).then((results) => browser.storage.local.remove(Object.keys(results)).then(() => browser.runtime.sendMessage({sdid, "progress": progress})).then(function (response) {if (! response) {throw null;}}).catch(() => browser.storage.local.set(Object.hasOwn(progress, "sections") ? results : {}).then(function () {download.className = "aborted";})));
      break;
  }
}
