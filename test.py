...
body2 = \
  ...
  '        if (connection) {\r\n' \
  '          connection.getSenders().forEach(function (sender) {sender.track.stop();});\r\n' \
  '        }\r\n' \
  '        document.getElementById("video").src = URL.createObjectURL(file);\r\n' \
  '        if (navigator.getAutoplayPolicy?.("mediaelement") == "disallowed") {alert("Allow media autoplay for the tab and reload !");}\r\n' \
  '      };\r\n' \
  ...
  '          const sender = connection.getSenders().find(({track: trck}) => trck?.readyState == "ended" && trck.kind == track.kind);\r\n' \
  '          if (sender) {\r\n' \
  '            sender.replaceTrack(track);\r\n' \
  '          } else {\r\n' \
  '            connection.addTrack(track, connection.stream);\r\n' \
  '          }\r\n' \
  '        });\r\n' \
  '      };\r\n' \
  '      const server = parent.server;\r\n' \
  ...
  '        if (! stream) {return;}\r\n' \
  '        stream.onremovetrack ??= function ({track}) {track.stop();};\r\n' \
