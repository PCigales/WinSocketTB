...
class TestRequestHandler(RequestHandler):
  def handle(self):
    ...
    if req.method not in {'GET', 'HEAD'} or (body := bodies.get(req.path.lower())) is None:
      return
    resp = \
      'HTTP/1.1 200 OK\r\n' \
      'Date: %s\r\n' \
...
gist = 'https://gist.githubusercontent.com/PCigales/dc41dc6f499bfd49019b503857c0f57f/raw/'
for n in 'ABCer':
  BA.set_credential(n, 'WebRTC', 'test')
bodies = {}
for names in (('wrtc_mes.html',), ('wrtc_vid.html',), ('wrtc_capt_e.html', 'wrtc_capt_r.html')):
  with WebRTCSignalingServer(9000, basic_auth=BA) as SignalingServer, TCPIServer(9001, TestRequestHandler, threaded=True):
    ...
    HTTPRequest = HTTPRequestConstructor(SignalingServer.idsocketgen)
    for name in names:
      print('http://127.0.0.1:9001/' + name)
      bodies['/' + name] = HTTPRequest(gist + name).body
      webbrowser.open('http://127.0.0.1:9001/' + name)
